...
<div class="max-w-2xl mx-auto px-4 py-8">
    <div class="mb-8">
        ...
        <h1 class="text-2xl font-bold text-gray-900">Tambah Armada Baru</h1>
    </div>

    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6" x-data="fleetForm()">
        <form action="{{ route('admin.fleets.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <input type="text" name="name" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="e.g. Avanza Veloz Platinum" required>
                </div>
                <div>
                    ...
                    <select name="type" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" required>
                        <option value="small">Small (4-5 Seats)</option>
                        <option value="standard">Standard (7 Seats)</option>
                        <option value="large">Large (Mini Bus/Elf)</option>
                    </select>
                </div>
            </div>
...
function fleetForm() {
    return {
        rows: [
            { seats: 1 }, // Driver/Front row usually has 1 passenger seat
            { seats: 3 },
            { seats: 3 }
        ],
        addRow() {
